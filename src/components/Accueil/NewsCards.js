...
const EnhancedNewsCards = () => {
  ...
  // Charger tous les posts WordPress au montage du composant
  useEffect(() => {
    loadPosts();
  }, []);
  ...
  const loadPosts = async () => {
    try {
      setLoading(true);
      // Récupérer tous les articles (max 100)
      const { items } = await fetchPosts({ perPage: 100, page: 1 });
      setPosts(items);
      setError(null);
    } catch (err) {
  ...
  // Mapping des catégories pour icônes et styles
  ...
  if (loading) return (
    <div className="container mx-auto px-4 py-12 text-center">Chargement…</div>
  );
  ...
  if (posts.length === 0) return (
    <div className="container mx-auto px-4 py-12 text-center">Aucune actualité pour le moment.</div>
  );
    ...
    <div className="container mx-auto px-4 py-12">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {posts.map((post) => {
          ...
          const img = featuredImageUrl(post) || defaultImg;
          ...
          return (
            <article
              key={post.id}
              className="relative bg-white rounded-2xl p-6 shadow-xl border border-blue-100 flex flex-col"
            >
              {/* Badge catégorie */}
              <div className="flex items-center justify-between mb-4">
                ...
                <categoryStyle.icon className="w-5 h-5 text-gray-600" />
              </div>
              ...
              />
              <p className="text-gray-600 leading-relaxed flex-1 text-sm mb-4 line-clamp-3">{excerpt}</p>
              ...
              <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                ...
                <span className="text-xs text-gray-500">{postDateLabel(post)}</span>
              </div>
            </article>
          );
        })}
